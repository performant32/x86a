...
This state is in charge of parsing directives, instructions, allocations for variables, etc  
If we come across a section directive we change the current output section  
by defualt we write to the `.text` section  
when we encouter an instruction mnemonic we look for any suitable instruction  
If found, we then parse any available arguments into an OperandValue which encodes the addressing mode and any symbol associated with it  

## Code Generation
After all necessary sections are generated for each file, we then create a .elf file corresponding to it  
These elf files are by defualt stored with 32 bit x86 code, designed to be linkable/compatible with conventional linkers such as ld  
